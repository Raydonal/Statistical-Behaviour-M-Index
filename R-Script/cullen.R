cullen <- function (data, discrete = FALSE, boot = NULL, method = "unbiased", 
          graph = TRUE, obs.col = "darkblue", obs.pch = 16, boot.col = "orange", main="") 
{
  ...
  if (graph) {
    ...
    if (!discrete) {
      ...
      s2 <- c(s2a, s2b)
      y <- c(ya, yb)
      polygon(s2, y, col = "grey94", border = "grey94")
      ...
      legend(xmax * 0.6, 0.86 * ymax, pch = 3, legend = "logistic", 
             bty = "n", cex = 0.8)
      legend(xmax * 0.6, 0.82 * ymax, fill = "grey94", 
      ...
      legend(xmax * 0.6, 0.98 * ymax, pch = 8, legend = "normal", 
             bty = "n", cex = 0.8)
      legend(xmax * 0.6, 0.94 * ymax, fill = "grey94", 
